...
inline bool KEY::is_pressed(){
    if(key == KEY_UP){
        if(CHECK_KEY_UP == 1) return true;
    } 
    else{
        if(CHECK_KEY0 == 0 || CHECK_KEY1 == 0 || CHECK_KEY2 == 0) return true;
    } 
    return false;
}
inline void KEY::exti_on(void (*func)()){
    delay_ms(10);
    if(is_pressed()) func();
}
...
void KEYStaticBuilder::EXTI0_KEY_UP(){
    key[3]->exti_on(LEDStaticBuilder::led1_on);
}

void KEYStaticBuilder:: EXTI4_KEY0(){
    key[0]->exti_on(LEDStaticBuilder::led1_off);
}

void KEYStaticBuilder::EXTI3_KEY1(){
    key[1]->exti_on(BEEPStaticBuilder::on);
}

void KEYStaticBuilder::EXTI2_KEY2(){
    key[2]->exti_on(BEEPStaticBuilder::off);
}
